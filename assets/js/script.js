...
// testimonials modal (optional; template-safe)
...
// custom select / project filter (optional)
...
const setActivePage = function (pageName) {
  ...
  for (let i = 0; i < pages.length; i++) {
    if ((pages[i].dataset.page || '').trim().toLowerCase() === normalized) {
  ...
  for (let i = 0; i < navigationLinks.length; i++) {
    const linkName = (navigationLinks[i].textContent || '').trim().toLowerCase();
    if (linkName === normalized) {
...
for (let i = 0; i < navigationLinks.length; i++) {
  navigationLinks[i].addEventListener('click', function () {
    setActivePage(this.textContent);
  });
}
